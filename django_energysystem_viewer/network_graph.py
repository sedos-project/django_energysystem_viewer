...
from .settings import STRUCTURE_FILE
...
def generate_trace(sector, algorithm, seperate_commodities):
    ...
    ----------
    sector: str
        The selected sector, which is used to filter the process set or in the
        case of aggregated commodities, a string of the selected sectors.
    algorithm: str
    ...
    updated_process_set = pd.read_excel(STRUCTURE_FILE, "Process_Set")
...
def generate_trace_process_specific(process_name, file):
    ...
    # load the process set, change the path if necessary
    updated_process_set = pd.read_excel(file, "Process_Set")
    ...
    # filter the process set for the selected process such that only the selected
    # process and its inputs and outputs are included
    ...
    for i, process in enumerate(processes):
        ...
        # right now, only single process names can be searched for, not multiple,
        # so the for loop is not necessary but kept for future use
        for input_node in input_list:
            input_node = input_node.strip()
            if input_node not in nodes:
                nodes.append(input_node)
        ...
        # the y-coordinates of the inputs and outputs should be that they are evenly distributed around 0,
        # which is the y-coordinate of the process node
...
def generate_trace_commodity_specific(commodity_name, selected_sectors, file):
    """
    ...
    # load the process set, change the path if necessary
    process_set = pd.read_excel(file, "Process_Set")
    ...
    for i, process in enumerate(processes):
        ...
        # if the commodity is an input of the process, the process is added to the nodes list and
        # the edge is added to the edges list
        # if the commodity is an output of the process, the process is added to the nodes list and
        # the edge is added to the edges list
    ...
    # the commodity node should be displayed in the center, the processes where it is an output
    # on the left and the processes where it is an input on the right
    # the appropriate coordinates have to be assigned to each node
    ...
    # we need two counters, one for the inputs and one for the outputs, to calculate the y-coordinates of
    # the processes, as i iterates over the node list
    j = 0
    k = 0
...
def assign_color(node, processes):
    ...
    For processes, the colors chosen are red for power, green for x2x, blue for industry,
    yellow for transport, magenta for heat.
    For inputs and outputs, the colors chosen are dark red for primary, red for secondary,
...
def generate_Graph(
    ...
    if not process_specific and not commodity_specific:
        ...
        if seperate_commodities == "sep":
            ...
            # if commodities are aggregated, the selected sectors are combined to one string
            # to use them in the generate_trace function
...
def get_processes() -> list[str]:
    """Creates the layout of the process search bar.
    ...
    pd.Series
        list of processes
    """
    process_set = pd.read_excel(STRUCTURE_FILE, "Process_Set")
    # Get unique processes from the DataFrame
    return list(process_set["process"].unique())


def get_commodities(selected_sectors):
    """Updates the commodity search bar according to the selected sectors.
    ...
    -------
    list
        The options of the commodity search bar.
    """
    if not selected_sectors:  # Check if no sectors are selected
        return []

    process_set = pd.read_excel(STRUCTURE_FILE, "Process_Set")

    # filter the process set for the selected sectors
    updated_process_set = process_set[process_set["process"].str.startswith(tuple(selected_sectors))]

    # get the inputs and outputs of the filtered process set
    sector_commodities = updated_process_set["input"].tolist() + updated_process_set["output"].tolist()
